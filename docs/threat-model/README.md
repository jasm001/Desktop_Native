...
| Administrador interno abusivo | alta | Las rutas `/admin/*` exigen ambiente `development`, flag explicito, rol local conocido y capability de lectura antes de resolver datos; principal separado del usuario de API y del agente; consultas limitadas a 25 registros, selecciones explicitas sin payload de auditoria y sin mutaciones; pruebas rechazan ambientes, roles y capabilities desconocidos, verifican ausencia de efectos laterales, cubren componentes del shell/tablas/estados vacios y recorren con Playwright acceso denegado y cuatro vistas en desktop/movil | OIDC/Entra, sesiones, RBAC/scopes reales, segregacion, alertas, auditoria de mutaciones y Playwright por rol productivo |
