...
| Administrador interno abusivo | alta | `/admin` exige ambiente `development`, flag explicito, rol local conocido y capability `portal.dashboard.read` antes de renderizar; principal separado del usuario de API y del agente; datos estaticos sin mutaciones; pruebas rechazan ambientes, roles y capabilities desconocidos | OIDC/Entra, sesiones, RBAC/scopes reales, segregacion, alertas, auditoria y Playwright por rol |
