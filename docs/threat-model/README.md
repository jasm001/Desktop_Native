...
| Fuga o abuso por Hermes local | alta | El proveedor Hermes de WinUI requiere flag explicito, API key en entorno local y endpoint `http`/`https` de loopback; no acepta `.env` del repositorio como fuente de secretos; limita el mensaje a 1000 caracteres; envia solo el texto escrito y un prompt informativo; no adjunta diagnosticos, tokens, historial completo, evidencia, hostnames ni datos corporativos; el historial visual queda en memoria del proceso WinUI; la salida no crea comandos, solicitudes, auditoria, outbox ni llamadas al DeviceAgent; las pruebas cubren proveedor deshabilitado y rechazo de endpoint no loopback | Revision AI/Security, reglas de datos, RAG curado, observabilidad, costos y proveedor aprobado antes de usar datos reales |
