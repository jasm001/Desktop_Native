...
- proveedor Hermes local opcional de WinUI, solo para texto libre informativo en
  demostracion.
...
| Fuga o abuso por Hermes local | alta | El proveedor Hermes de WinUI requiere flag explicito, API key en entorno local y endpoint `http`/`https` de loopback; no acepta `.env` del repositorio como fuente de secretos; limita el mensaje a 1000 caracteres y el tiempo de espera cliente a 120 segundos; envia solo el texto escrito y un prompt informativo; no adjunta diagnosticos, tokens, historial completo, evidencia, hostnames ni datos corporativos; el historial visual queda en memoria del proceso WinUI; la salida no crea comandos, solicitudes, auditoria, outbox ni llamadas al DeviceAgent; las pruebas cubren proveedor deshabilitado, timeout configurado y rechazo de endpoint no loopback | Revision AI/Security, reglas de datos, RAG curado, observabilidad, costos y proveedor aprobado antes de usar datos reales |
...
| Administrador interno abusivo | alta | Las rutas `/admin/*` exigen ambiente `development`, flag explicito, rol local conocido y capability de lectura antes de resolver datos; principal separado del usuario de API y del agente; consultas limitadas a 25 registros, selecciones explicitas sin payload de auditoria y sin mutaciones; el esqueleto adicional usa estados sinteticos en memoria para acceso, aprobaciones, soporte y reportes; pruebas rechazan ambientes, roles y capabilities desconocidos, verifican ausencia de efectos laterales, cubren componentes del shell/tablas/estados vacios y recorren con Playwright acceso denegado y ocho vistas en desktop/movil | OIDC/Entra, sesiones, RBAC/scopes reales, segregacion, alertas, auditoria de mutaciones y Playwright por rol productivo |
